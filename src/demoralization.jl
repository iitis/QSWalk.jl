...
julia> full(QSWalk.default_local_hamiltonian(4))
...
Value of `vertexset` should be generated by `demoralized_lindbladian` in order
...
"""
    ...
    make_vertex_set(revincidence_list)
...
julia> QSWalk.make_vertex_set(vset)()
...
```
"""
function make_vertex_set(revincidence_list::Vector{Vector{Int}})
...
"""
    ...
    demoralized_lindbladian(A[, lindbladians][, epsilon])
...
function demoralized_lindbladian(A::SparseDenseMatrix,
  ...
  vset = make_vertex_set(revincidence_list)
...
function demoralized_lindbladian(A::SparseDenseMatrix;
                                epsilon::Real=eps())
  vset = make_vertex_set(reversed_incidence_list(A, epsilon=epsilon))
  degrees = [length(v) for v=vset()]

  demoralized_lindbladian(A, Dict( d=>fourier_matrix(d) for d=degrees), epsilon=epsilon)
end
